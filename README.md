...
풀이 : https://blog.naver.com/ndb796/221234424646

최단 거리 문제인데, 다른 문제와는 다르게 간선 별로 거리 값이 주어져 있다. 즉, 먼저 탐색되는 값이 최소 비용의 값이 아닐 수 있다.

처음에 BFS로 접근해봤는데, 제일 처음 탐색되었을 때 그 값을 최단 거리로 인식할 경우 최소 비용을 찾기 곤란하다.(위에 이유로)

따라서, 위 최소 비용 문제(앞으로 이렇게 부르기) 는 다익스트라 알고리즘으로 풀어야 하는데, 이 문제는 그 예제로 매우 적합하다.

요약 : 다익스트라 알고리즘 대표 문제임.

------------------------ 2022/05/23 ------------------

2018 KAKAO BLIND RECRUITMENT 2번 : 다트 게임 (Level 1)

문제 : https://programmers.co.kr/learn/courses/30/lessons/17682

조건 분기 때문에 애먹었다. 그래도 잘 풀었는데 문제 조건을 잘못 읽는 바람에 고생했다.. 인터넷 보고 1줄 고치니 잘 돌아가더라.
...
풀이 : https://seongonion.tistory.com/127

다른사람 코드를 보고 이해했다.. 간단해 보이지만 생각해내기 어려웠던 것 같다.
...
문제 풀이 시간과 입력 갯수를 보고 ( o(nlogn)으로 풀어야함 ) 우선순위 큐로 풀어야 함을 알아야 한다. (우선순위 큐 : o(logn) )

우선순위 큐를 이용한 풀이의 발상은 항상 해내기가 힘들다,,
...
풀이 : https://2hs-rti.tistory.com/87

간단해 보이지만 문제 조건 분기, 빡센 구현으로 역대급 난이도가 나온것 같다. 구현 문제를 연습하고 싶으면 다시 풀어보자..
...
풀이 : https://velog.io/@zer0silver/%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98-%EC%9C%A0%EB%8B%88%EC%98%A8-%ED%8C%8C%EC%9D%B8%EB%93%9C

-----  https://ssungkang.tistory.com/198

-----  https://t-anb.tistory.com/45

그리디 o(n^2) 풀이로는 풀 수 없고, 시간 최적화를 위해 유니온 파인드 알고리즘을 활용해야한다. (링크 참고)

● 유니온 파인드 알고리즘 정리

- 재귀 호출이 끝나고 부모 테이블 값을 바로 갱신해주어서 ( "경로 압축"을 해주어서 ) 시간 복잡도를 개선한 것이 핵심
...
풀이 : https://journeytosth.tistory.com/16

PS를 몇달 놓다보니까 머리가 굳은것같다.. 정렬 메커니즘에서 힌트를 얻을 수 있다.
...
풀이 : https://chocochip101.tistory.com/entry/백준-12904번-파이썬-A와-B

https://conkjh032.tistory.com/320

이 문제를 통해 배운 아이디어는 거꾸로 생각해보기.

A -> B의 경우의 수를 따질때는 여러 경우의수를 생각해야 해서 시간초과가 나지만,

B -> A로 가는 케이스를 거꾸로 계산해보면 O(N)의 시간으로 컴팩트하게 풀 수 있따
